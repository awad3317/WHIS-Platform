          ...
          <div class="flex grow flex-col items-center justify-between lg:flex-row lg:px-6">
              ...
              <div :class="menuToggle ? 'flex' : 'hidden'"
                  ...
                  <div class="2xsm:gap-3 flex items-center gap-2">
                      <!-- Language Toggler -->
                      <form action="{{ route('switch.lang') }}" method="POST" class="inline">
                          @csrf
                          <button type="submit" name="lang"
                              value="{{ Session::get('locale') === 'en' ? 'ar' : 'en' }}"
                              class="hover:text-dark-900 relative flex h-11 w-11 items-center justify-center 
               rounded-full border border-gray-200 bg-white text-gray-500 transition-colors 
               hover:bg-gray-100 hover:text-gray-700 
               dark:border-gray-800 dark:bg-gray-900 dark:text-gray-400 
               dark:hover:bg-gray-800 dark:hover:text-white">
                              <span class="text-sm font-semibold">
                                  {{ strtoupper(Session::get('locale') === 'en' ? 'EN' : 'AR') }}
                              </span>
                          </button>
                      </form>
                                  ...
                                  <li>
                                      <a class="flex gap-3 rounded-lg border-b border-gray-100 p-3 px-4.5 py-3 hover:bg-gray-100 dark:border-gray-800 dark:hover:bg-white/5"
                                          ...
                                          <span class="relative z-1 block h-10 w-full max-w-10 rounded-full">
                                              <img src="{{ asset('tailadmin/build/src/images/user/user-02.jpg') }}"
                                                  alt="المستخدم" class="overflow-hidden rounded-full" />
                                              <span
                                                  class="bg-success-500 absolute left-0 bottom-0 z-10 h-2.5 w-full max-w-2.5 rounded-full border-[1.5px] border-white dark:border-gray-900"></span>
                                          </span>

                                          <span class="block">
                                              <span
                                                  class="text-theme-sm mb-1.5 block text-gray-500 dark:text-gray-400">
                                                  <span class="font-medium text-gray-800 dark:text-white/90">تيري
                                                      فرانسي</span>
                                                  يطلب الإذن لتغيير
                                                  <span class="font-medium text-gray-800 dark:text-white/90">المشروع -
                                                      تطبيق نغانتر</span>
                                              </span>
                  ...
                  <div class="relative" x-data="{ dropdownOpen: false }" @click.outside="dropdownOpen = false">
                      <a class="flex items-center text-gray-700 dark:text-gray-400" href="#"
                          ...
                          <div
                              class="flex items-center text-gray-700 dark:text-gray-400 {{ app()->getLocale() == 'ar' ? 'flex-row-reverse' : '' }}">
                              <span
                                  class="h-11 w-11 overflow-hidden rounded-full {{ app()->getLocale() == 'ar' ? 'ml-3' : 'mr-3' }}">
                                  <img src="{{ asset('tailadmin/build/src/images/user/owner.jpg') }}"
                                      alt="{{ app()->getLocale() == 'ar' ? 'المستخدم' : 'User' }}" />
                              </span>
                              <span
                                  class="text-theme-sm block font-medium {{ app()->getLocale() == 'ar' ? 'ml-1' : 'mr-1' }}">
